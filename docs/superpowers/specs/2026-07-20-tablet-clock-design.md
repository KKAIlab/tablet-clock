...
## 定稿 v8-final 2026-07-21（点睛三笔）

参考《论阴阳与时间的关系》（用户提供），三处修正、零添饰：
1. 子午线消长：阳值 = (1-cos(π·h/12))/2，子时阴极、午时阳极，
   一日一息连续消长；卯时/酉时恰为 0.5，天然满足"卯酉线分昼夜"
2. 画龙点睛：秒线太极眼改朱砂——浓墨已逝、淡墨未至，唯"当下"一点是活的红
3. 顺时针即流逝：番茄环已逝之弧改为从正上方顺时针扫过
第二轮（减法）：专注 8 秒后操作提示收敛为「专 注 中」三字。
第三轮：克制，不再加笔。定稿。
