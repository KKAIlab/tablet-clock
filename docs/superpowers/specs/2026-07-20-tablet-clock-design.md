...
## 真机根因终判 2026-07-21（已修复，用户确认）

iPad 实测"点击无反应"的最终根因**不是**触摸事件丢失（诊断面板证实
btn pointerup 60/60 全达），而是按钮处理函数第一行 `ensureAudio()` 在
iPadOS 26 桌面模式 Safari 上抛异常，令 `dispatch('toggle')` 永不可达。
v5 修复：先 dispatch 后音频 + 音频全程 try/catch。
教训：装饰性功能（音频/动效/上报）必须放在主流程之后并静默降级；
远程排障时，让用户设备自己开口（诊断面板计数 + err 显示）胜过一切猜测。

## 迭代记录 2026-07-21c（水墨版 v6）

参考《水墨丹青为何总在呼吸？》（用户提供）：
- 配色整体退蓝入墨：夜=青墨池，昼=宣纸；RGB 插值过中性灰即"墨分五色"
- 呼吸柔光改为双椭圆错位复合的"墨晕"；body::before 加宣纸噪点肌理
- 全局唯一色彩=朱砂：▶ 按钮改方圆角"朱印"，番茄钟环与结束呼吸皆朱砂
- 图标同步换水墨风（宣纸底/墨圈/朱砂时针/角上朱印）
